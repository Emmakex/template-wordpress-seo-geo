...
Current execution pointer: **10E.2A.5.3.2.2 — private payload extraction + full checksum replay in Migration Bridge 0.8.29.** Migration Bridge 0.8.28 is accepted on `main` at `79f143483c141a03c608e71bc4fe25eef0441b25`: the private local handoff is now staged into a deterministic child `import` job and passes the existing `ImportPreflight` against destination authority derived from the isolated runtime, while destination tables/uploads/themes remain absent and restore stays locked. 0.8.29 reuses the existing `ImportPayloadVerifier` on that child job, extracts only into private import workspace storage in bounded resumable batches, revalidates the frozen parent target/archive authority around each batch and unlocks restore eligibility only after the full replayed checksum equals the frozen package checksum. **No target restore occurs in 0.8.29.** The next microphase is **10E.2A.5.3.3.1 — transactional database staging restore**, reusing `ImportDatabaseRestorer`.
...
- WordPress Smoke CI `36223562551` passed, including child-import authority binding, zero destination tables/uploads/themes, target-mutation rejection and archive/child-state drift invalidation;
