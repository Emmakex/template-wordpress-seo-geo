...
- **10E.2A.5.5.2 — guarded finalization plan (0.8.33 candidate):** reuse the accepted `ImportFinalizationPlanner` on the bound private same-server child, revalidate fresh local target authority, fingerprint rewritten DB/file staging, and freeze deterministic activation/rollback table + file maps while creating no target tables, candidates or rollback roots;
- **10E.2A.5.5.3 — reversible local database activation:** next; adapt the accepted Portable Import database activator to the isolated local target while keeping file promotion disabled and rollback available.
