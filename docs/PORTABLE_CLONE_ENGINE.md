...
- **10E.2A.5.4.2 — private file staging restore (complete in 0.8.31):** reuse the accepted 10E.2A.4.4 `ImportFileRestorer` on the same child job, require completed local database staging, copy uploads/plugins/themes only into private job staging, retain per-file `files-meta` hashes plus an independent verification pass, and prove that target client roots remain unpromoted;
- **10E.2A.5.4 — isolated local restore:** complete through 5.4.2; no active target table activation or client-file promotion occurs;
- **10E.2A.5.5.1 — serialization-safe environment rewrite in staging (complete in 0.8.32):** reuse the accepted `ImportEnvironmentRewriter` on verified local staging, bind every bounded batch to frozen parent authority, rewrite only supported job-owned staging tables, preserve credential-like values, and require an idempotent second pass with zero rewritable source URLs;
...
The 5.3.2.2 payload step reuses `ImportPayloadVerifier` unchanged for extraction and checksum semantics, wrapped by local-clone parent authority. Each batch starts from a still-valid 5.3.2.1 state; completion requires the child payload state to be `complete`, the replayed checksum to equal the frozen package checksum and a fresh local destination preflight to remain valid. The child may then expose `restore_allowed=true` exactly as Portable Import does, but this is eligibility only: no target table, uploads, plugins or themes are restored in this microphase. A changed target/runtime/package/archive invalidates the parent and revokes the child restore gate before local orchestration may proceed.
...
The 5.5.1 rewrite step reuses `ImportEnvironmentRewriter` rather than adding local-clone search/replace code. It can start only while the parent handoff, payload checksum, database staging and file staging all remain verified. Supported staging tables are rewritten transactionally in bounded row batches; PHP serialized containers are decoded with classes disabled, JSON is recursively transformed, credential-like option keys are skipped, and active WordPress `home`/`siteurl` values are explicitly proven unchanged by acceptance. Completion requires a second idempotent pass with `verify_source_urls=0`; even then the target remains unactivated and client files remain unpromoted. Parent authority drift invalidates the ready state and revokes child restore eligibility.

The 5.5.2 finalization step reuses `ImportFinalizationPlanner` for deterministic database/file fingerprints and immutable activation/rollback planning. For `private-same-server`, its safety gate resolves the future active roots under the verified isolated target rather than the control-plane WordPress, and it independently revalidates `LocalCloneTargetPreflight` on every bounded batch. The plan requires future target tables to remain absent, stages to stay under the job-owned namespace, rollback table names to stay under the isolated prefix, and file candidate/rollback siblings to stay inside the target `wp-content` filesystem. Completion means hashes and paths are frozen only: no table is renamed, no candidate directory is created and no client file is promoted.


The 5.5.3 database-activation step reuses `ImportDatabaseActivator` and keeps the source WordPress as the control plane. For a bound `private-same-server` child, the activator resolves the isolated destination `options` table from the accepted target prefix rather than `$wpdb->options`, prepares an external recovery journal before mutation, and revalidates the frozen finalization hash immediately before the swap. A transaction applies only the target-staging safety overlay (`home`/`siteurl` already rewritten, `blog_public=0`, Migration Bridge control plugin active); one atomic multi-table `RENAME TABLE` then activates all target tables. Verification checks layout, row counts and control-option hashes. Client uploads/plugins/themes remain unpromoted, and atomic reverse rename is available until file promotion begins.


The 5.5.4 file-promotion step keeps control-plane and target runtime deliberately separate. In `private-same-server` mode, the generic promotion planner resolves active roots from the isolated destination path and reads runtime state from the activated target `wp_options`; the promoter writes active_plugins/template/stylesheet back to that same validated target table in a bounded transaction and never calls source `update_option()`. Candidates are copied from private staging and fingerprinted before any root swap. After the reversible same-filesystem swap, the active target is re-hashed to the frozen file fingerprint and must expose the source plugin/theme runtime plus Migration Bridge while retaining `blog_public=0`. Rollback restores the pre-promotion control roots/runtime and resets the DB journal so database rollback can proceed. Completion establishes local-clone handoff readiness but still requires final target smoke/reporting before the isolated clone is treated as accepted.

The 5.5.5 handoff step is deliberately read-only for the promoted target. It accepts only a still-verified local file-promotion journal, then rechecks each activated target table against its frozen row count, verifies destination `home`/`siteurl`, `blog_public=0`, exact plugin/theme runtime, Migration Bridge control files, active file fingerprint, source isolation and both file/database rollback availability. The report stores bounded identity/counter/hash evidence rather than client content and derives a deterministic SHA-256 excluding timestamps. A saved handoff is reusable only if a fresh evidence pass reproduces that hash; file/database rollback or later target drift therefore invalidates it automatically. Sandbox hardening remains active and this phase does not authorize production cutover.
