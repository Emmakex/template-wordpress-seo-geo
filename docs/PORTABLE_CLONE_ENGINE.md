...
- **10E.2A.5.3.1 — private same-server package handoff (0.8.27 candidate):** reuse the accepted private ZIP builder against the already verified `local-clone` workspace while preserving the exact frozen package manifest/hash, keeping the clone job active and binding the private archive SHA-256/bytes to current ownership, sandbox-control and package authority;
- **10E.2A.5.3.2 — target intake + sandbox preflight:** consume only the accepted private handoff artifact through the existing Portable Import validation/extraction primitives and repeat the sandbox destination guard before any restore mutation;
