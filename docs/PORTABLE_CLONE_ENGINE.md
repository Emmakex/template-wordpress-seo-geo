...
- **10E.2A.4.4 — file restore:** complete in 0.8.18; manifest-backed uploads/plugins/themes are restored only into `import/staged-files`, never active `wp-content`, and a second independent SHA-256 traversal reconciles exact file/byte totals.
- **10E.2A.4.5 — serialization-safe environment rewrite:** active in 0.8.19; rewrite source home/site URLs and prefix-sensitive WordPress role/capability keys only inside deterministic staging tables. PHP serialized values are parsed and rebuilt with corrected byte lengths without instantiating imported objects; canonical post GUID values remain unchanged; opaque custom serialization requiring replacement blocks; a second bounded verification pass must find no remaining source environment values.
