...
- **10E.2A.5.3.2.2 — private payload extraction + full checksum replay (0.8.29 candidate):** reuse `ImportPayloadVerifier` on the deterministic child job in bounded file/byte batches, keep extraction inside private job storage, revalidate parent local-clone authority before and after progress, require an exact frozen package checksum plus fresh destination preflight before the child restore gate opens, and revoke that gate if target/package/archive authority drifts;
- **10E.2A.5.3.2 — target intake + sandbox preflight:** active through 5.3.2.2; payload verification may expose restore eligibility only to the child import job while active destination tables/uploads/themes remain untouched;
- **10E.2A.5.3 — local package handoff + sandbox preflight:** active through 5.3.2.2;
- **10E.2A.5.4.1 — transactional database staging restore:** next; reuse the accepted 10E.2A.4.3 database restore primitive under the local-clone parent authority, writing only job-owned staging tables.
