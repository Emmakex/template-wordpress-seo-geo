...
Status: **active — 10E.2A.4.1 through 10E.2A.4.3 are accepted; 10E.2A.4.4 destination file staging is the Migration Bridge 0.8.18 candidate**
...
- **10E.2A.4.4 — file restore:** active in 0.8.18; copy only verified `uploads/plugins/themes` payload into a protected deterministic job-owned destination staging tree. Each source file must match its exported metadata bytes/SHA-256 before atomic staging; root/global counts and bytes must reconcile exactly. Active WordPress file roots are not overwritten and activation/swap remains outside this subphase.
