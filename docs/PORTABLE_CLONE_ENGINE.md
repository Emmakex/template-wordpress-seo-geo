...
- **10E.2A.5.5.1 — serialization-safe environment rewrite in staging (0.8.32 candidate):** reuse the accepted `ImportEnvironmentRewriter` on verified local staging, bind every bounded batch to frozen parent authority, rewrite only supported job-owned staging tables, preserve credential-like values, and require an idempotent second pass with zero rewritable source URLs;
- **10E.2A.5.5.2 — guarded finalization plan:** next; reuse the accepted Portable Import finalization planner before any database activation or file promotion.
