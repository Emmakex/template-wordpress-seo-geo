...
Migration Bridge 0.8.34 implements phase 10E.2A.5.5.3 with reversible local database activation. It reuses the accepted `ImportDatabaseActivator` on the verified private same-server child, freezes an external activation journal from the accepted 0.8.33 plan, overlays only the control/runtime options needed to keep the isolated target safe and bootable, and promotes all verified staging tables through one atomic `RENAME TABLE` map under the isolated target prefix. Post-swap verification binds the activated database back to the frozen plan/fingerprint while production WordPress and client file roots remain untouched. An explicit rollback reverses the atomic table map back to staging and invalidates the parent activation state so a fresh finalization plan is required before retry. Next: reversible local file promotion plus final isolated-target verification.
