...
Migration Bridge 0.8.26 starts phase 10E.2A.5.2.2.2.1 with a bounded control-runtime install. It requires the accepted 0.8.25 core runtime plus the verified ownership marker, creates only `wp-content/plugins/seo-geo-migration-bridge/`, copies the currently running Bridge source in resumable batches through `ExportWorkspace`, and runs a second exact topology/file/byte/SHA-256 verification pass. Uploads, themes, unrelated plugins, `wp-config.php` and database rows remain untouched. The next microphase creates the isolated target configuration and active sandbox hardening.
