...
Migration Bridge 0.8.29 continues phase 10E.2A.5.3.2 with bounded private payload extraction + full checksum replay. It reuses the accepted `ImportPayloadVerifier` against the deterministic child `import` job created by 0.8.28, revalidates the frozen local-clone target/archive authority before and after each batch, and writes extracted payload only under private import workspace storage. Completion requires the child payload checksum to equal the frozen package checksum and fresh `ImportPreflight` to mark the child `payload-verified`/`restore_allowed=true`; this is restore eligibility only. Destination tables, uploads and themes remain untouched until the next transactional staging-restore microphase.
