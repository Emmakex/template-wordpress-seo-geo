...
final class LocalCloneSandboxRuntimeBootstrapper {
	...
	private function wp_config_content( array $state ): ?string {
		...
		foreach ( $constants as $name => $value ) {
			$lines[] = "define( '" . $name . "', " . $this->php_string_literal( $value ) . ' );';
		}
		...
		foreach (
			...
			$lines[] = "define( '" . $salt_name . "', " . $this->php_string_literal( $derived ) . ' );';
		}

		$lines[] = '';
		$lines[] = '$table_prefix = ' . $this->php_string_literal( $target_prefix ) . ';';
		$lines[] = "define( 'WP_HOME', " . $this->php_string_literal( $target_url ) . ' );';
		$lines[] = "define( 'WP_SITEURL', " . $this->php_string_literal( $target_url ) . ' );';
	...
	/**
	 * Encode one exact PHP single-quoted string literal for generated config.
	 *
