	...
	private function normalize( string $job_id, array $state ): ?array {
		...
		foreach ( is_array( $state['roots'] ?? null ) ? array_slice( $state['roots'], 0, 3 ) : array() as $root ) {
			...
			$roots[] = array(
				...
				'copied_files'    => max( 0, (int) ( $root['copied_files'] ?? 0 ) ),
				'copied_bytes'    => max( 0, (int) ( $root['copied_bytes'] ?? 0 ) ),
				'verified_files'  => max( 0, (int) ( $root['verified_files'] ?? 0 ) ),
				'verified_bytes'  => max( 0, (int) ( $root['verified_bytes'] ?? 0 ) ),
