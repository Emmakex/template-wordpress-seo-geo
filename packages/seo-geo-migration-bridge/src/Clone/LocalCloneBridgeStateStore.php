<?php
/**
 * Portable Clone Migration Bridge control-runtime bootstrap state.
 *
...
/**
 * Stores bounded resumable state for the local-clone Bridge control runtime.
 */
final class LocalCloneBridgeStateStore {
	...
	/**
	 * Return one normalized Bridge runtime state.
	 *
	...
	/**
	 * Persist one bounded Bridge runtime state.
	 *
	 * @param string              $job_id Clone job identifier.
	...
	/**
	 * Delete one Bridge runtime state.
	 *
	 * @param string $job_id Clone job identifier.
	 */
	...
	/**
	 * Normalize one Bridge runtime state.
	 *
	 * @param string              $job_id Clone job identifier.
	...
	private function normalize_state( string $job_id, array $state ): ?array {
		...
		if ( ! in_array( $stage, array( 'bridge-copy', 'bridge-verify', 'bridge-complete' ), true ) ) {
			return null;
		}
		...
		return array(
			...
			'source_main_sha256'       => $this->normalize_hash( $state['source_main_sha256'] ?? '' ),
			...
			'bridge_runtime_ready'     => true === ( $state['bridge_runtime_ready'] ?? false ),
	...
	/**
	 * Normalize one directory cursor name.
	 *
