<?php
/**
 * Portable Clone local finalization plan state.
 *
...
/**
 * Stores bounded parent state for local-clone finalization planning.
 */
final class LocalCloneFinalizationPlanStateStore {
	public const OPTION_NAME    = 'seo_geo_migration_local_clone_finalization_plan_v1';
	...
	private function normalize_state( string $job_id, array $state ): ?array {
		...
		if (
			...
			|| ! in_array( $stage, array( 'database-fingerprint', 'file-fingerprint', 'ready' ), true )
		) {
			return null;
		}
		...
		return array(
			...
			'activation_plan_ready'        => true === ( $state['activation_plan_ready'] ?? false ),
			'target_unactivated'           => true === ( $state['target_unactivated'] ?? false ),
			'finalization_next'            => $this->bounded_code( $state['finalization_next'] ?? '' ),
