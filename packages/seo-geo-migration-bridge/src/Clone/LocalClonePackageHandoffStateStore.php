<?php
/**
 * Portable Clone private local-package handoff state.
 *
...
/**
 * Stores bounded state for a same-server private handoff archive.
 */
final class LocalClonePackageHandoffStateStore {
	...
	/**
	 * Return one normalized handoff state.
	 *
	...
	/**
	 * Persist one bounded handoff state.
	 *
	 * @param string              $job_id Clone job identifier.
	...
	private function normalize_state( string $job_id, array $state ): ?array {
		...
		return array(
			...
			'sandbox_config_sha256'   => $this->normalize_hash( $state['sandbox_config_sha256'] ?? '' ),
			'sandbox_mu_sha256'       => $this->normalize_hash( $state['sandbox_mu_sha256'] ?? '' ),
			...
			'handoff_next'            => $this->bounded_code( $state['handoff_next'] ?? '' ),
