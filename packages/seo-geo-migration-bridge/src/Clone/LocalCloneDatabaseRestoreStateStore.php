<?php
/**
 * Portable Clone local database staging state.
 *
...
/**
 * Stores bounded parent state for local-clone transactional database staging.
 */
final class LocalCloneDatabaseRestoreStateStore {
	public const OPTION_NAME    = 'seo_geo_migration_local_clone_database_restore_v1';
	...
	private function normalize_state( string $job_id, array $state ): ?array {
		...
		$stage  = is_string( $state['stage'] ?? null ) ? $state['stage'] : 'schema';
		if (
			! in_array( $status, array( 'running', 'ready', 'blocked' ), true )
			|| ! in_array( $stage, array( 'schema', 'rows', 'table-verify', 'complete' ), true )
		) {
			return null;
		}
		...
		return array(
			...
			'target_tables_untouched'      => true === ( $state['target_tables_untouched'] ?? false ),
			'client_content_untouched'     => true === ( $state['client_content_untouched'] ?? false ),
			'database_next'                => $this->bounded_code( $state['database_next'] ?? '' ),
	...
	private function normalize_identifier( mixed $value ): string {
		return is_string( $value ) && 64 >= strlen( $value ) && 1 === preg_match( '/^[A-Za-z0-9_]*$/', $value )
			? $value
			: '';
	}
